...
import { X, MapPin, Clock, QrCode } from 'lucide-react'
...
import { startUsingSeat, endUsingSeat } from '@/app/actions/seat'
import { useRouter } from 'next/navigation'
import type { SeatWithReservation, SeatReservation } from '@/app/actions/seat'

interface SeatsPageClientProps {
  initialSeats: SeatWithReservation[]
  currentUserSeat: SeatReservation | null
  currentUserId: string
}

// Convert DB seat to component-friendly format
function convertSeat(seat: SeatWithReservation) {
  return {
    id: seat.id,
...
export function SeatsPageClient({
  initialSeats,
  currentUserSeat,
  currentUserId,
}: SeatsPageClientProps) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()
  const [selectedFloor, setSelectedFloor] = useState('2')
  const [selectedSeat, setSelectedSeat] = useState<ReturnType<typeof convertSeat> | null>(null)
  const [showQRView, setShowQRView] = useState(false)
  const [qrSeat, setQrSeat] = useState<ReturnType<typeof convertSeat> | null>(null)

  // Convert seats for display
  const seats = useMemo(() => initialSeats.map(convertSeat), [initialSeats])

  // Get current user's seat
  const myCurrentSeat = useMemo(() => {
    if (currentUserSeat && currentUserSeat.seat) {
      return {
        ...
        reservationId: currentUserSeat.id,
      }
    }
    return null
  }, [currentUserSeat])
  ...
  const handleStartUsing = async (seatId: string) => {
    if (myCurrentSeat) {
    ...
    startTransition(async () => {
      try {
        await startUsingSeat(seatId)
        toast.success('좌석 사용이 시작되었습니다')
        setSelectedSeat(null)
        router.refresh()
      } catch (error: any) {
        toast.error(error.message || '좌석 사용 시작에 실패했습니다')
      }
    })
  }

  const handleEndUsing = async (reservationId: string) => {
    startTransition(async () => {
      try {
        await endUsingSeat(reservationId)
        toast.success('좌석 반납이 완료되었습니다')
        setSelectedSeat(null)
        router.refresh()
      } catch (error: any) {
        toast.error(error.message || '좌석 반납에 실패했습니다')
      }
    })
  }

  // Get available floors from data
  const availableFloors = useMemo(() => {
    const floors = new Set(seats.map((s) => {
      const match = s.location.match(/^(\d+)층/)
  ...
  return (
    <div className="space-y-6">
        ...
        >
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex-shrink-0">
              <Button
                onClick={() => myCurrentSeat.reservationId && handleEndUsing(myCurrentSeat.reservationId)}
                disabled={isPending}
                style={{
      ...
      {/* Floor Selector Tabs */}
      <Tabs defaultValue={availableFloors[0] || '2'} value={selectedFloor} onValueChange={setSelectedFloor} className="w-full">
        <TabsList
          className="w-full"
          style={{
                  ...
                  <CardContent className="p-6">
                    ...
                    >
                      <div className="text-center">
                        ...
                        <Button
                          onClick={() => {
                            const testSeat = seats.find((s) => s.status === 'available' && s.location.startsWith(`${floor}층`))
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="pt-4 border-t" style={{ borderColor: 'var(--border)' }}>
                          <div className="flex items-center justify-between">
                            <span style={{ fontSize: 'var(--font-size-caption)', lineHeight: 1.4, color: 'var(--muted-foreground)' }}>상태</span>
                        ...
                        {selectedSeat.status === 'in_use' && selectedSeat.currentUserId === currentUserId && selectedSeat.reservationId && (
                          <div className="pt-4 space-y-3">
                            <Button
                              className="w-full"
                              variant="outline"
                              onClick={() => selectedSeat.reservationId && handleEndUsing(selectedSeat.reservationId)}
                              disabled={isPending}
                              style={{
          ...
          <CardContent className="p-6">
            <QRSeatReservation
              seat={{
                id: qrSeat.id,
                name: qrSeat.name,
                location: qrSeat.location,
                status: qrSeat.status,
                currentUserId: qrSeat.currentUserId,
                startTime: qrSeat.startTime,
              }}
              currentUserName="사용자"
        ...
        <CardContent className="p-6">
          ...
          <SeatListView
            ...
            myCurrentSeat={myCurrentSeat ? {
              id: myCurrentSeat.id,
              name: myCurrentSeat.name,
              location: myCurrentSeat.location,
              status: myCurrentSeat.status,
              currentUserId: myCurrentSeat.currentUserId,
              startTime: myCurrentSeat.startTime,
              reservationId: myCurrentSeat.reservationId,
            } : undefined}
